<template>
  ...
  <script setup>
import { ref } from "vue";
import { login } from "@/services/api"; // Importamos la función de login

const email = ref("");
const password = ref("");
const message = ref("");
const loading = ref(false); // Estado de carga para mejorar UX
...
const logIn = async () => {
  ...
  try {
    ...
    message.value = error.message || "Error al iniciar sesión. Verifica tus credenciales.";
  } finally {
    loading.value = false;
  }
};
